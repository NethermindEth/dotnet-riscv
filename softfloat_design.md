...
Не покрыто слоем 1 (сознательно): возврат *скалярного* double уходит в `fa0`
через `ReturnTypeDesc`. Под слоем 2 скалярных FP-типов в IR не остаётся, и
вопрос исчезает; если слой 1 понадобится изолированно — гейтится ещё
`ReturnTypeDesc`/`genReturn`.

Флаг по умолчанию не выставлен → патч инертен для текущих сборок.
Кто выставляет: ilc — при отсутствии `f` в instruction sets таргета
(`--instruction-set=-f,-d`, см. патчи 16/17) передаёт `SOFTFP_ABI` в
CORJIT_FLAGS. Это отдельный маленький патч в ilc
(`ILCompiler.Compiler`, `CorInfoImpl` jit flags).
...
Порядок подачи в апстрим: (1) instruction sets F/D/C/A, (2) патч 26 —
мал и самодостаточен, (3) 27+28 — после design-issue («soft-float RISC-V
target»; в репозитории уже есть прецедент soft-float RISC-V в Mono:
